...
# Train x-vector extractor (2_xvect_extr)
...
# Train peech synthesis (SS) acoustic model (3_ss_am)

if [ $stage -le 1 ] && false; then
  printf "${GREEN}\nStage 1: Training Speech Synthesis AM model.${NC}\n"
  local/vc/am/00_run.sh ${data_netcdf}
fi


# Train  Neural source filter (NSF) model (4_nsf)
