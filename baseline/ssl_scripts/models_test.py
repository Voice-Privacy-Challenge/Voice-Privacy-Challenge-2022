...
class latentGenerator(Generator):
    def __init__(self, h):
        super().__init__(h)
        if h.get('w2v2_pt_model_path',None):
            self.feat_model, cfg, task = fairseq.checkpoint_utils.load_model_ensemble_and_task([str(h.w2v2_pt_model_path)])
            self.feat_model = self.feat_model[0]
            set_parameter_requires_grad(self.feat_model,
                                        h.get('ssl_freeze', None))  # ssl_freeze=True not finetune ssl model
        ...
        self.context = h.get('context', None)
    ...
    def forward(self, **kwargs):
        ...
        #audio_data [batchsize,1,audio_len]
        if self.context:
