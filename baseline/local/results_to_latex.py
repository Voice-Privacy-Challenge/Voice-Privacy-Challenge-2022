...
def parse_scores(scores_file):  
    ...
    combined_ASV = combine_tables_asv(ASV_table_dev,ASV_table_test,'ASV',header_combined_ASV_table, nr_cols_combined_ASV,width_combined_ASV)
...
def combine_tables_asv(table1,table2,task,header,nr_cols,width):
    if (len(table1)>12) & (len(table2)>12) & (len(table1)==len(table2)): #(12 is the size of an empty table)
        combined_table=initialise_table(nr_cols,width)
        combined_table.append(header)
        content_table1 = table1[7:-5]
        content_table2 = table2[7:-5]
        ...
        combined_table[4] =combined_table[4][:26]+'|'+combined_table[4][26:33]+'|'+combined_table[4][33:]       
...
def generate_asr_line(data_info,numbers):
    ...
    wer_line = ' & '.join([dataset, data, "{:.3f}".format(float(wer_small)), "{:.3f}".format(float(wer_large))]) + '\\\\ \hline '+'\n'
    return wer_line
