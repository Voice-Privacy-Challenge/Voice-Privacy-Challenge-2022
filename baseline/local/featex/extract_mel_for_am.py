...
def extract_mel_API(input_file, output_file, sp):
    """extract_mel_API(input_file, output_file, sp)
    input_file: str, path to the input waveform
    output_file: str, path to the output mel-spec
    sp: instance of SpeechProcessing
    """

    if os.path.isfile(input_file):

        # if necessary, configure the STFT and Mel-spectrogram configuration here
        #sp = SpeechProcessing(fftl=1024, fl=400, fs=160)
...
if __name__ == "__main__":
    ...
    # claim one API instance
    sp = SpeechProcessing(fftl=1024, fl=400, fs=160)
    # produce mel-spectogram output
    extract_mel_API(sys.argv[1], sys.argv[2], sp)
    # return
